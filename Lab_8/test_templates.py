...
def test_index_template_loop():
    """Тест цикла в index.html"""
    template = env.get_template('index.html')

    currencies = [
        {'id': 1, 'cur': 'USD', 'date': '2024-01-01', 'value': 90.5},
        {'id': 2, 'cur': 'EUR', 'date': '2024-01-01', 'value': 98.2}
    ]
    ...
    # Проверяем что данные из цикла есть в результате
    assert 'USD' in result
    assert 'EUR' in result
    assert '90.5' in result or '90,5' in result  # Может быть запятая
    assert '98.2' in result or '98,2' in result
